...
export type Schema = [string, FieldDefinition];
...
const typeSizes: Record<FieldType, number> = {
    "string": 0,
    "uint8": 1, "int8": 1,
    "uint16le": 2, "uint16be": 2,
    "int16le": 2, "int16be": 2,
    "uint32le": 4, "uint32be": 4,
    "int32le": 4, "int32be": 4,
    "float32le": 4, "float32be": 4,
    "float64le": 8, "float64be": 8
};

function resolveFieldDefinition(def: FieldDefinition): [number, FieldType, boolean] {
    if (typeof def === "number") {
        const isRaw = def < 0;
        return [Math.abs(def), "string", isRaw];
    }

    return [typeSizes[def], def, false];
}

function readField(buf: Buffer, offset: number, len: number, type: FieldType, isRaw: boolean = false): string | number {
    switch (type) {
        ...
        case "int16le": return buf.readInt16LE(offset);
        case "int16be": return buf.readInt16BE(offset);
        case "uint32le": return buf.readUInt32LE(offset);
        case "uint32be": return buf.readUInt32BE(offset);
        case "int32le": return buf.readInt32LE(offset);
        case "int32be": return buf.readInt32BE(offset);
        case "float32le": return buf.readFloatLE(offset);
        case "float32be": return buf.readFloatBE(offset);
        case "float64le": return buf.readDoubleLE(offset);
        case "float64be": return buf.readDoubleBE(offset);
        default: {
...
function writeField(value: unknown, len: number, type: FieldType, isRaw: boolean = false): Buffer {
    const buf = Buffer.alloc(len, 0x20); // 0x20 = space

    switch (type) {
        case "uint8": buf.writeUInt8(Number(value ?? 0), 0); break;
        case "int8": buf.writeInt8(Number(value ?? 0), 0); break;
        case "uint16le": buf.writeUInt16LE(Number(value ?? 0), 0); break;
        case "uint16be": buf.writeUInt16BE(Number(value ?? 0), 0); break;
        case "int16le": buf.writeInt16LE(Number(value ?? 0), 0); break;
        case "int16be": buf.writeInt16BE(Number(value ?? 0), 0); break;
        case "uint32le": buf.writeUInt32LE(Number(value ?? 0), 0); break;
        case "uint32be": buf.writeUInt32BE(Number(value ?? 0), 0); break;
        case "int32le": buf.writeInt32LE(Number(value ?? 0), 0); break;
        case "int32be": buf.writeInt32BE(Number(value ?? 0), 0); break;
        case "float32le": buf.writeFloatLE(Number(value ?? 0), 0); break;
        case "float32be": buf.writeFloatBE(Number(value ?? 0), 0); break;
        case "float64le": buf.writeDoubleLE(Number(value ?? 0), 0); break;
        case "float64be": buf.writeDoubleBE(Number(value ?? 0), 0); break;
        default: {
